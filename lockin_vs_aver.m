...
noise = [];
for i = 1:Npt
    idx = [1, sort(randperm(npt-2,poissrnd(250))+1), npt];
    nos = nan(1, npt);
...
IlockinGEF = sqrt(f2 .^2 + g2 .^2);

fltr = 1;
                                        % -- SIN --
lockin = fft(f1, [], 2);                % filtering
lockin(:, fltr+1:end-fltr) = 0;  

lockin = real(ifft(lockin, [], 2));     % restoring
l = mean(lockin, 2);

                                        % -- COS --
lockinA = fft(g1, [], 2);               % filtering
lockinA(fltr+1:end-fltr) = 0;

lockinA = real(ifft(lockinA, [], 2));   % restoring
lA = mean(lockinA, 2);      

Ilockin = sqrt(l .^2 + lA .^2);
...
plot(Delta, Ilockin ./max(Ilockin) .*max(I), "DisplayName", "lockin FFT", "LineWidth", 1); 
plot(Delta, IlockinGEF ./max(IlockinGEF) .*max(I), "DisplayName", "lockin Int", "LineWidth", 1); 
